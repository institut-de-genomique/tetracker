...
# s'il ne contient qu'une ligne et que l'alignement est dans XA pour l'un des deux: récrire l'alignement principal avec les infos de XA et écrire.
	...
	for my $j ( 0 .. $#{@{$pairs->[1]}} ) {
	    ...
	    if ( $LIB eq "MP" && $pairs->[0]->[$i]->[0] eq $pairs->[1]->[$j]->[0] && ( ($pairs->[0]->[$i]->[1] > $pairs->[1]->[$j]->[1] && $pairs->[0]->[$i]->[2] eq "+" && $pairs->[1]->[$j]->[2] eq "-"  &&  $pairs->[0]->[$i]->[1] - $pairs->[1]->[$j]->[1] < $SPAN[0]*2 ) || ($pairs->[0]->[$i]->[1] < $pairs->[1]->[$j]->[1] && $pairs->[0]->[$i]->[2] eq "-" && $pairs->[1]->[$j]->[2] eq "+" && $pairs->[1]->[$j]->[1] - $pairs->[0]->[$i]->[1] < $SPAN[0]*2) )  ){
