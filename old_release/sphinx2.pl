...
close(IN);
#my @sTE=sort {$a->[1] cmp $b->[1] || $b->[2] <=> $a->[2]} @TE;
my @sTE=@TE;
undef(@TE);

# 2. open fasta reference and construct database
open ( IN, "/env/cns/proj/projet_AKL/scratch/synthetic_epiRIL/c1c5_COPIA93x80/in.fa" ) || die "Cannot open reference: $!\n";
my %faindx;
my $currchr="";
my $newchr;
...
#	print ("TSD $tsd\n");
	...
	$nextevent->[2]+= $event->[5]-$event->[4]+1 if($nextevent->[1] eq $event->[1] && $nextevent->[2] > $event->[2]+$event->[5]-$event->[4] && $nextevent->[0] ne "D");
