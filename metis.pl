...
our $TR_ERROR="";
our $REFERENCE="";
our ( $BAM , $INPUT_DIR , $LIB , $EXT , $MAX_COV , $COV_EXTENSION , $READ_SIZE , $SAMTOOLS_PATH , $DOC_MAD , $SPAN_MAD , $SAMPLE_SIZE , $SLC , $MIN_READS, $OUTPUT_DIR, $NOSORT, $NOLUTHER, $MINCOV, $CLSTPRM, $ANNOTATE, $PICARD, $SCORE_DONORS, $BEDTOOLS_PATH, $DONOR_ANNOT, $ACCEPTOR_ANNOT, $SA) = ( "", "" , "MP" , 20000 , 1000, 500, 76, "/env/cns/src/samtools/samtools-0.1.8/bin/samtools" , 4 , 4 , 1000000 , "/env/cns/proj/projet_AKL/slc/slclust", 10, "", "", "", 255, "", "", "~agilly/picard-tools-1.65", "", "/env/cns/src/BEDTools/BEDTools-Version-2.16.2/bin", "/env/cns/proj/projet_AKL/db/TAIR10te.bed", "/env/cns/proj/projet_AKL/TAIR-Complete.bed", "");
my $MAN="";
my @args=@ARGV;
...
sub usage{
    print STDERR "\nERROR:\tMandatory parameters were not supplied. Be sure you specify at least -in, -out and one of the postprocessing parameters.\n";
    print "\t\t(arguments supplied were \'", join "\n\t\t\t\t\t   ", @args, "\'.)\n\n" ;
    print STDERR "Usage :\ttetracker metis -discordant [bamfile] -in [in_dir] -out [out_dir]\n";
    print STDERR "\t\t\t-annotate -acc_annot [annot_file] -donor_annot [annot_file]\n";
    print STDERR "\t\t\t-score_donors -reverse_transcriptase\n\n";
    print STDERR "\t Please run \'tetracker metis -man\' for a detailed explanation of command-line options.\n\n";
    exit();
}
...
sub man{
    my $BASEDIR=`pwd`;
    chomp $BASEDIR;
    system("man $BASEDIR/man/metisman 2>/dev/null");
...
#	print "accid=$accid, data1=$data[1]\n";
	if($data[1] == $accid){	$donors{"$data[7]:$data[8]-$data[9]"}=$line;}
	    ...
	    if(scalar(keys(%donors))==1 && $SA eq "") {print OUTSTR $donors{(keys(%donors))[0]}, "\t*\n";%donors=();$donors{"$data[7]:$data[8]-$data[9]"}=$line;$accid++;}else{
		my @d=keys(%donors);
